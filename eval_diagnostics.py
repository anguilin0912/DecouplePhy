...
def run_diagnostics(dataset_name):
    print("="*70)
    print(f"[INFO] Evaluating DecouplePhy Framework | Dataset: {dataset_name}")
    print("="*70)

    model_search = glob.glob(os.path.join(SCRIPT_DIR, "config", "**", f"*{dataset_name}*.model"), recursive=True)
    if not model_search: raise FileNotFoundError(f"[ERROR] Model file for {dataset_name} not found.")
    ...
    # Force Heatmap Capture Engine
    heatmap_atk = None 
    heatmap_max_magnitude = 0.0

    print(f"[INFO] Streaming test data and extracting dual-track residuals...")
                ...
                if alert:
                    ...
                    if len(buffer_list) > 0:
                        with torch.no_grad():
                            ...
                            # Force capture the most intense residual window during an alert
    ...
    # List to store raw text strings for the log file
    raw_text_log = []
    ...
    # Terminal Report
    print("\n" + "="*70 + f"\n[RESULT] Quantitative Evaluation Report: {dataset_name}\n" + "="*70)
    ...
    # 1. Export Pure Text Log
    text_log_path = os.path.join(OUTPUT_DIR, f"RCA_Event_Log_{dataset_name}.txt")
    with open(text_log_path, "w", encoding="utf-8") as f:
        f.write("\n".join(raw_text_log))
        
    # 2. Export Quantitative Report (JSON format for numbers)
    report_path = os.path.join(OUTPUT_DIR, f"Appendix_Report_{dataset_name}.json")
    report_data = {
    ...
    print(f"\n[SUCCESS] Appendix data successfully saved to: \n -> {text_log_path}\n -> {report_path}")

    # 3. Export Academic Heatmap
    if heatmap_atk is not None:
        print(f"[INFO] Generating high-resolution academic residual heatmap...")
        plt.rcParams.update({'font.family': 'sans-serif', 'font.sans-serif': ['DejaVu Sans', 'Arial', 'Helvetica'], 'axes.linewidth': 1.2})
        plot_sensors_count = min(15, len(model.sensor_order))
        df_res = pd.DataFrame(data=heatmap_atk.T[:plot_sensors_count, :], index=model.sensor_order[:plot_sensors_count], columns=[f"t-{model.window_size - i}" for i in range(model.window_size)])
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.heatmap(df_res, ax=ax, cmap='YlOrRd', annot=False, linewidths=0.5, linecolor='black', cbar_kws={"label": 'Absolute Residual Magnitude'})
        ax.set_title(f'Spatial-Temporal Residual Map: {dataset_name}', fontsize=14, fontweight='bold', pad=15)
        ax.set_ylabel('Monitored System Components', fontsize=12, labelpad=10)
        ax.set_xlabel('Time Window Sequence', fontsize=12, labelpad=10)
        ...
        pdf_path = os.path.join(OUTPUT_DIR, f"Result_{dataset_name}_Academic.pdf")
        plt.savefig(pdf_path, format='pdf', dpi=300)
        print(f"[SUCCESS] Academic Visualization successfully saved to: \n -> {pdf_path}")
